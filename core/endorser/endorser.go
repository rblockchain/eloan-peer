...
//xx	"github.com/hyperledger/fabric/core/scc/rbcnotify"
...
func NewEndorserServer() pb.EndorserServer {
	...
	e.endorserChan = make(chan *common.RBCMessage, 200)
...
//rongzer,王剑，接收交易，交通过orderer分发
func (e *Endorser) SendTransaction(ctx context.Context, rbcMessage *common.RBCMessage) (*pb.ProposalResponse, error) {
	if rbcMessage == nil || rbcMessage.Data == nil {
		err := fmt.Errorf("SendTransaction Message is nil")
		return nil, err
	}
	response := &pb.ProposalResponse{Response: &pb.Response{Status: 200, Message: "send transaction success"}}

	//通道初始化消息
	if rbcMessage.Type == 0 {
		endorserLogger.Infof("receive init chain transaction")

		//发送给Orderer
		res, err := service.GetGossipService().SendToOrderer(rbcMessage.ChainID, rbcMessage)
		if err != nil {
			endorserLogger.Errorf("send exec transaction to orderer %v,err:%s", res, err)
			return nil, err
		}
	...
	//获取块消息
	if rbcMessage.Type == 1 {
		endorserLogger.Infof("receive get block msg")

		//首先尝试从本地节点获取块信息，否则从orderer获取
		lgr := peer.GetLedger(rbcMessage.ChainID)
		if lgr != nil {
			bnum, err := strconv.ParseUint(rbcMessage.Extend, 10, 64)
			if bnum >= 0 && err == nil {
				block, err := lgr.GetBlockByNumber(bnum)
				if block != nil && err == nil {
					blockBuf, err := proto.Marshal(block)
					if err == nil && blockBuf != nil { //block异常
						bcRes := &ab.BroadcastResponse{Status: common.Status_SUCCESS}

						bcRes.Data = blockBuf
						buf, err := proto.Marshal(bcRes)
						if err == nil && buf != nil {
							response.Payload = buf
							return response, nil
						}
	...
	//Peer接收背书消息
	if rbcMessage.Type == 3 {
		endorserLogger.Infof("receive endorser transaction %s", rbcMessage.TxID)
	...
	//运行背书消息
	if rbcMessage.Type == 4 {
		endorserLogger.Debugf("receive endorser msg txid:%s", rbcMessage.TxID)

		e.endorserChan <- rbcMessage
	}

	//接收Orderer过来的交易错误通知
	if rbcMessage.Type == 7 {
		endorserLogger.Infof("receive Rejection txid:%s", rbcMessage.TxID)

		producer.Send(producer.CreateRejectionEvent(rbcMessage.ChainID, rbcMessage.TxID, string(rbcMessage.Data)))
	}

	//Http转发至Orderer请求消息
	if rbcMessage.Type == 21 {
		//发送给Orderer
		res, err := service.GetGossipService().SendToOrderer(rbcMessage.ChainID, rbcMessage)
		if err != nil {
			endorserLogger.Errorf("send exec transaction to orderer %v,err:%s", res, err)
...
func (e *Endorser) EndorserProposal(rbcMessage *common.RBCMessage, ch chan int) {
	...
	err := proto.Unmarshal(rbcMessage.Data, signedProp)
	if err != nil {
		endorserLogger.Errorf("EndorserProposal rbcMessage Unmarshal err:%s", err)
		<-ch
		return
	}
	// at first, we check whether the message is valid
	prop, hdr, hdrExt, err := validation.ValidateProposalMessage(signedProp)
	if err != nil {
		endorserLogger.Errorf("EndorserProposal rbcMessage validation err:%s", err)
		<-ch
		return
	}

	chaincode.EndorserChan.Lock()
	runStatus := chaincode.EndorserChan.M[rbcMessage.ChainID+":"+hdrExt.ChaincodeId.Name]
	chaincode.EndorserChan.Unlock()
	//正常运行中
	if runStatus == 1 {
		go e.processEndorser(rbcMessage, signedProp, prop, hdr, hdrExt, ch)
	} else {
		endorserLogger.Infof("single processEndorser rbcMessage txid:%s", rbcMessage.TxID)

		e.processEndorser(rbcMessage, signedProp, prop, hdr, hdrExt, ch)
	}

}

func (e *Endorser) processEndorser(rbcMessage *common.RBCMessage, signedProp *pb.SignedProposal, prop *pb.Proposal, hdr *common.Header, hdrExt *pb.ChaincodeHeaderExtension, ch chan int) {
	res, err := e.processEndorserExec(signedProp, prop, hdr, hdrExt)
	<-ch

	var rbcMessage1 *common.RBCMessage
	if err != nil {
		//chaincode.EndorserChan.M[rbcMessage.ChainID+":"+hdrExt.ChaincodeId.Name] = 0
		//endorserLogger.Errorf("processEndorser rbcMessage Exec err:%s", err)
		//发送错误信息至endorser
		errInfo := err.Error()
		...
		//rbc执行错误反馈
		rbcMessage1 = &common.RBCMessage{Type: 6, Data: []byte(errInfo), ChainID: rbcMessage.ChainID, TxID: rbcMessage.TxID}
	} else {
		chaincode.EndorserChan.Lock()
		chaincode.EndorserChan.M[rbcMessage.ChainID+":"+hdrExt.ChaincodeId.Name] = 1
		chaincode.EndorserChan.Unlock()
	}
	...
	if rbcMessage1 == nil {
		buf, err := proto.Marshal(res)
		if err != nil {
			endorserLogger.Errorf("processEndorser rbcMessage Marshal err:%s", err)
			return
		}
		rbcMessage1 = &common.RBCMessage{Type: 5, Data: buf, ChainID: rbcMessage.ChainID, TxID: rbcMessage.TxID}
	}

	res1, err := service.GetGossipService().SendToOrderer(rbcMessage.ChainID, rbcMessage1)
	if err != nil {
		endorserLogger.Errorf("processEndorser rbcMessage SendToOrderer err:%s", err)
		return
	}
	endorserLogger.Debugf("send endorser Msg To Orderer txid :%s,res:%s", rbcMessage.TxID, res1)

}

// ProcessProposal process the Proposal
func (e *Endorser) processEndorserExec(signedProp *pb.SignedProposal, prop *pb.Proposal, hdr *common.Header, hdrExt *pb.ChaincodeHeaderExtension) (*pb.ProposalResponse, error) {
	...
	if chainID != "" {
		...
		if _, err := lgr.GetTransactionByID(txid); err == nil {
			return nil, fmt.Errorf("TransactionExist") //这个错误存在特殊处理！！！
		}
