...
func (vm *DockerVM) createContainer(ctxt context.Context, client dockerClient,
	...
	dockerLogger.Debugf("Create container: %s", imageID)
...
func (vm *DockerVM) Start(ctxt context.Context, ccid ccintf.CCID,
	...
	fmt.Printf("\nexecute args1:%s, %s,%s, spec:%v,%v,%v,%s", imageID, containerID, strings.Join(args, ","), ccid.ChaincodeSpec, ccid.ChaincodeSpec.Type, pb.ChaincodeSpec_JAVA, ccid.ChaincodeSpec.Type == pb.ChaincodeSpec_JAVA)
	...
	if ccid.ChaincodeSpec.Type == pb.ChaincodeSpec_JAVA && len(args) > 5 && args[4] == "chaincode.jar" {
		...
		dockerLogger.Infof("execute args4:%s", javaEnv)
		//imageID = javaEnv
	}
