...
type deliverServiceImpl struct {
	...
	stopping       bool
	//rongzer,王剑增加
	ordererConnects map[string]orderer.AtomicBroadcastClient
}
...
func NewDeliverService(conf *Config) (DeliverService, error) {
	ds := &deliverServiceImpl{
		...
		//rongzer,王剑增加
		ordererConnects: make(map[string]orderer.AtomicBroadcastClient),
	}
